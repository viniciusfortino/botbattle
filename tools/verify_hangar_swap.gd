...
func _initialize() -> void:
	...
	hangar.loadout.mount("arm_left/main/rail_1", load("res://content/catalog/parts/plasma_cannon.tres"))
	hangar._refresh()
	await process_frame
	await create_timer(0.3).timeout
	_shot("hangar_plasma")

	hangar.loadout.mount("arm_left/main", load("res://content/catalog/parts/heavy_arm.tres"))
	hangar._refresh()
	await process_frame
	await create_timer(0.3).timeout
